<template>
  <div :class="[prefixCls + '-table', prefixCls + '-table-'+ size, loading && (prefixCls + '-table-loading')]">
        ...
        <slot name="head-row" :columns="columns">
          <tr>
            <th v-if="rowSelection" :class="[prefixCls + '-table-selection-column']">
              <input v-if="dataList && dataList.length" type="checkbox"
                     v-bind="{checked:isCheckedAll,disabled:isDisabledAll}" @change="onCheckAll"
              />
            </th>
            ...
            <th v-for="(column, index) in columns" :width="column.width">
              ...
              <dropdown ref="filterMenu" v-if="column.filters" trigger="hover">
                ...
                <div name="dropdown-menu" slot="dropdown-menu"
                     :class="[prefixCls + '-dropdown-menu', prefixCls + '-table-filter-dropdown']">
                  <template v-if="column.filterMultiple !== false">
                    ...
                    <div :class="[prefixCls + '-table-filter-dropdown-btns']">
                      ...
                      <a :class="[prefixCls + '-table-filter-dropdown-link', prefixCls + '-table-clear']"
                         @click="resetFilter(column)">重置</a>
                    </div>
                  </template>
                  <template v-else>
                    <ul :class="[prefixCls + '-table-filter-single']">
                      <li
                        :class="[prefixCls + '-table-filter-single-item', column.__selectedText === '全部'?'selected':'']"
                        @click="resetFilter(column)">
                        <icon type="tick" size="10"></icon>
                        <span>全部</span>
                      </li>
                      <li v-for="filter in column.filters" @click="onFilter(true, column, filter.value, filter.text)"
                          :class="[column.__selectedText === filter.text?'selected':'', prefixCls + '-table-filter-single-item']">
                        <icon type="tick" size="10"></icon>
              ...
              <div v-if="dataList && dataList.length && column.sorter" :class="[prefixCls + '-table-sorter']">
                <icon type="caretup" size="10" @click.native="sortAction(column, index, 'ascend')"
                      :class="[sorderOrder[index] == 'ascend' && (prefixCls + '-table-active')]"></icon>
                <icon type="caretdown" size="10" @click.native="sortAction(column, index, 'descend')"
          ...
          <slot name="row" :record="record" :row-index="row-index">
            <tr :track-by="rowIndex" @click="onRowClick(rowIndex, record)">
              <td v-if="rowSelection" :class="[prefixCls + '-table-selection-column']">
                <input type="checkbox" v-model="checkedValues" :value="record[rowKey]"
                       @change.stop="onCheckOne($event,record)"
                       v-bind="rowSelection.getCheckboxProps && rowSelection.getCheckboxProps(record)"
                />
              </td>
              <td v-if="expandedRowRender" :class="[prefixCls + '-table-row-expand-icon-cell']">
                <span v-if="!record.__no_expand"
    ...
    <div v-show="pagination && pagination.total > 0" :class="[prefixCls + '-table-pagination']">
      <pagination ref="pager" :total="pagination.total" :page-size="pagination.pageSize" :show-jumper="true"
                  :show-size-changer="true" @pagination-page-change="changePage"
                  @pagination-size-change="changeSize"></pagination>
    </div>
  </div>
</template>
